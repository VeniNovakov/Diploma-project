...
const ChangeAmount: React.FC<ChangeAmountProps> = ({item, className, amount, setAmount}) => {
  ...
  const removeAmount = () => {
    ...
    if(instanceOfProduct(item)){
      ...
      setTempProduct({...tempProduct, addOns: [...tempProduct.addOns?.filter(addOn => {
        return addOn.id !== item.id;
      })]})

      setIsDisabled(true);
    }else if(!instanceOfProduct(item) && amount >= 1){
      setTempProduct({...tempProduct, addOns: [...tempProduct.addOns?.map(addOn => {
        if(addOn.id === item.id){
          return {
            ...addOn,
            amount
  ...
  const addAmount = () => {
    ...
    if(!instanceOfProduct(item) && amount === 1){
      setTempProduct({...tempProduct, addOns:[...tempProduct.addOns, {...item, amount}]})
    }else if(amount >= 1){
      setTempProduct({...tempProduct, addOns:[...tempProduct.addOns?.map(addOn => {
        if(addOn.id === item.id){
          return {
...
const ProductPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (firstRender.current || !hasOldEntry || tempProductFromCookie.product.id !== numericId - 1) {
      Cookies.set('tempProduct', JSON.stringify({ addOns: [], product: menu[numericId - 1], amount: 1 }));
  ...
  return (
    <ProductProvider>
      <Link to='/menu' className='absolute right-0 mr-4 border hover:bg-slate-300'>
        <button>X</button>
      </Link>
...
const Product = () => {
  ...
  return (
    <div className="flex flex-col border border-gray-300 rounded p-4 max-w-md self-end">
      ...
      <p className="text-blue-500 font-bold">{round(tempProduct.product?.price * tempProduct.amount + total * tempProduct.amount, 2)}</p>
    </div>
  );
};
